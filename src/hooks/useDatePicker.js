...
import { getAvailableMonths, getAvalibleDaysOfWeek, getAvailableYears } from "../utils/date";
...
const useDatePicker = (language) => {
    ...
    const getPrevSlicedMonth = () => {
        ...
        const prevMonthDays = [];
        const daysInPrevMonth = new Date(currentMonth.year, currentMonth.number - 1, 0).getDate(); // Returns date of last month day (if  0 is las day of month)
    ...
    const getNextSlicedMonth = () => {
        const remainingDays = 7 * rowsCount - (currentMonth.firstDayOfMonthIndex + currentMonth.numberOfDays);
    ...
    const nextMonth = () => {
        if (currentMonth.number === 12) {
            ...
            setCurrentMonth(new Month(new Date(currentMonth.year, currentMonth.number + 1 - 1), language));
        }
    };
    ...
    const goToDate = (year, month) => {
        const monthIndex = typeof month === "string" ? avalibleMonth.indexOf(month) : month - 1; // Якщо число, то використовуємо його
